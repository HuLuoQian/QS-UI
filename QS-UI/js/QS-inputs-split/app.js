import { log } from '@/QS-UI-CONFIG/index.js';
import { getSendData, getAsyncDataFc } from '@/QS-UI-CONFIG/components/QS-inputs-split/js/picker.js';
import interfaces from '@/QS-UI-CONFIG/components/QS-inputs-split/js/interfaces';
import { inputCustomTapFc, inputCustomTapCatchFc, filterTypeObj, verifyTypeObj } from '@/QS-UI-CONFIG/components/QS-inputs-split/js/input.js';
import { UpLoadFile, pics_splice } from '@/QS-UI-CONFIG/components/QS-inputs-split/js/pics';
import sendSMS from '@/QS-UI-CONFIG/components/QS-inputs-split/js/sms';
import { errObj } from '@/QS-UI-CONFIG/components/QS-inputs-split/js/Msg.js';
				...
				Promise.all(promiseArr).then(res=>{
					...
					for(let i = 0; i < newArr.length; i++) {
						...
						if(Array.isArray(item)) {
							const [err, suc] = item;
							newArr[i].upLoadResult = suc;
						}else{
							newArr[i].upLoadResult = item;
						}
					}
					resolve(newArr);
				}).catch(err=>{
	...
	return new Promise(async (resolve, reject) => {
		try{
			for (const o in obj) {
				...
				// formData赋值
				if(!returnObj.formData) returnObj.formData = {};
				switch (type){
					case typeObj.picker_custom:
						returnObj.formData[o] = returnObj.data[o].data;
						break;
					case typeObj.picker_custom2:
						returnObj.formData[o] = returnObj.data[o].data;
						break;
					case typeObj.picker_date:
						returnObj.formData[o] = returnObj.data[o].data;
						break;
					case typeObj.picker_city:
						returnObj.formData[o] = returnObj.data[o].data;
						break;
					default:
						returnObj.formData[o] = returnObj.data[o];
						break;
				}
			}
			if(returnObj.verifyErr && returnObj.verifyErr.length>0) {
				const currentVerifyErr = returnObj.verifyErr[0];
				let errText = currentVerifyErr.title || '';
				switch (currentVerifyErr.checkErrType[0]){
					case errTypeObj.required:
						errText += '不能为空';
						break;
					default:
						errText += '校验失败';
			...
			//TODO handle the exception
			reject({errInfo: '程序错误', verifyErr:[e]});
		}
	})
}
