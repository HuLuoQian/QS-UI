<template>
	<QSPickerTemplate ref="QSPickerTem" :height="height" :fontScale="fontScale" :buttonSet="buttonSet" :title="title"
		...
		<block v-if="linkage">
			<block v-if="linkageNum==2">
				<picker-view @touchmove.prevent.stop="voidFc()" indicator-style="height: 60px;" :value="value" @change="bindChange($event)">
					<picker-view-column>
						<view class="flex_row_c_c pickerDefault" :class="((value[0] || 0) === index?'selectColor':'defaultColor')" v-for="(item,index) in (itemObject.step_1 || [])"
						 :key="index">
		...
		computed: {
			...
			getItem3() {
				if (this.async) {
					...
					return (this.itemObject.step_3 ? this.itemObject.step_3[this.value[0] || 0] ? this.itemObject.step_3[this.value[0] ||
						0][this.value[1] || 0] : [] : []) || [];
				}
			}
		},
