...
module.exports = function (data, fields, deepClone = true) {	//.链式字符串获取对象属性方法
	let d = nGetField(data, fields);
	if(d && isObject(d)) {
		if(d.hasOwnProperty('_vnode')) return d;	//如果是vue实例则直接返回
	}
	try{
		return (d && (isObject(d) || isArray(d)) && deepClone)?JSON.parse(JSON.stringify(d)):d;
	}catch(e){
...
const nGetField = function(data, fields, pattern) { //字符串.链式获取指定字段数据， 摘自网上，，，
